...
do
local SUDO = 237327088 --put your id here(BOT OWNER ID)
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
   ...
   local text = '😐♨️😐♨️Full name : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'😐♨️Username: '..Username..'\n'
               ..'😐♨️ID : '..result.id..'\n\n'
  local hash = '😐♨️rank:'..extra.chat2..':variables'
  local value = redis:hget(hash, result.id)
  if not value then
	 if result.id == tonumber(SUDO) then
	   text = text..'Rank : Executive sudo \n\n'
